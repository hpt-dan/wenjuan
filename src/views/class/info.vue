  ...
  .departInfo(v-loading="allLoadint")
    my-table(:table="table" ref="myTable")
      ...
      span(slot="ctrl-button")
        el-button(type="primary" @click="goAdd" size="mini") 添加班级信息
    el-drawer( title="单位信息"  :visible.sync="show" direction="rtl" :wrapperClosable="false")
      el-form(:model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm")
        ...
        el-form-item(label="所属学校:" prop="parentId")
          el-select(v-model="ruleForm.parentId " placeholder="所属学校")
            el-option(v-for="(item ,index) in dept" :key="index" :value="item.id" :label="item.label")
        el-form-item(label="班级名称:" prop="label")
          el-input.dd(v-model="ruleForm.label" placeholder="班级名称")
        //el-form-item(label="单位类型:" prop="type")
          el-input.dd(v-model="ruleForm.type" placeholder="单位类型")
        el-form-item(label="班级人数:" prop="num")
          el-input-number.dd(v-model="ruleForm.num" placeholder="班级人数" :min="1" :controls="false" :max="999999")
